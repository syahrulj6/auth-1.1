...
import { compileActivationTemplate, compileResetPasswordTemplate, sendMail } from '@/lib/mail';
...
type ActivateUserFn = (jwtUserId: string) => Promise<'userNotExist' | 'alreadyActivated' | 'success'>;
...
type ResetPasswordFn = (jwtUserId: string, password: string) => Promise<'userNotExist' | 'success'>;

export const resetPasswordActions: ResetPasswordFn = async (jwtUserId, password) => {
  const payload = verifiedJwt(jwtUserId);
